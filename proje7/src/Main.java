...
public class Main {
    public static void main(String[] args) {
        ...
        // 3) Somut sınıflar ve nesne oluşturma
        // VideoContent: id, title, duration, basePrice, quality
        ...
        VideoContent v3 = new VideoContent("V3", "Funny Cat Video", 2, 0.0, 480); // Free

        // PodcastContent: id, title, duration, basePrice, live, totalAds
        PodcastContent p1 = new PodcastContent("P1", "Tech Talk", 45, 50.0, false, 5);
        PodcastContent p2 = new PodcastContent("P2", "Live News", 60, 50.0, true, 10); // Live -> 0 ads

        // EBookContent: id, title, duration, basePrice, pageCount
        EBookContent e1 = new EBookContent("E1", "Learn Java", 120, 80.0, 400); // >300 pages
        ...
        System.out.println("Total Revenue (No Discount Policy): " + manager.totalRevenueIfAllBought());
        ...
        System.out.println("Setting WeekendDiscountPolicy (10% off)");
        manager.setPricingPolicy(new WeekendDiscountPolicy());
        System.out.println("Total Revenue (Weekend Policy): " + manager.totalRevenueIfAllBought());
        // Verify individual price with policy
        // We can't easily verify individual via manager.printAll() unless we read output, but total revenue change confirms it.
        manager.printAll();
    }
}

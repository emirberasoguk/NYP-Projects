...
public class Main {
  public static void main(String[] args) {
    ...
    CargoBot cargo = new CargoBot("Laz Ziyanın Botu", "KB001", 500.0);
    GroundDrone drone = new GroundDrone("UcanVızıltı", "UV001", 8);

    // 2. MODÜL OLUŞTURMA VE EKLEME
    NavigationModule nav1 = new NavigationModule("NavCore", "2.1", "HarikalarDiyarı 2024");
    SafetyModule safe1 = new SafetyModule("KalkanlıKoruma", "1.5", 3);
    ...
    // Note: The prompt example only explicitly adds to 'car', but asks for
    // "CargoBot ve Drone'a da modüller ekle (benzer şekilde)"
    // I will add some dummy modules to them to be safe and consistent with a
    // "complete" setup,
    // although the output example only shows 'car' modules in the final block.
    // Wait, the output example "MODULE INFORMATION" only loops over
    // 'car.getModules()'.
    // So adding modules to others is good practice but not strictly required for
    // the output matching.
    // I'll stick to the explicit instructions.

    // 3. AI MODEL GÜNCELLEME TESTİ (Overloading)
    ai1.updateModel("EmirAI-2"); // sessiz güncelleme
    ai1.updateModel("EmirAI-3", true); // verbose güncelleme
 ...
 * - Amaç: Üst sınıfın (Vehicle) constructor'ını çağırarak ortak field'ları
 * initialize etmek
 * - Kural: super() çağrısı constructor'ın ilk satırı olmalı
 * 
 * B) OVERRIDE EDİLEN METOT İÇİNDE:
 * - Örnek: Car.getStatus() → return super.getStatus() + ", Passengers: " +
 * passengerCapacity
 * - Amaç: Üst sınıfın metodunu çağırıp üzerine ek bilgi eklemek, kod tekrarını
 * önlemek
 ...
 * - Çözüm: COMPILE-TIME (Derleme zamanında hangi metodun çağrılacağı
 * belirlenir)
 * 
 * OVERRIDING (Alt sınıf, aynı imza):
 * - Car, CargoBot, GroundDrone: getStatus() metodunu override eder
 * - NavigationModule, SafetyModule, AIControlModule: getInfo() metodunu
 * override eder
 * - Engineer, Technician, Manager: describe() metodunu override eder
 * - Çözüm: RUNTIME (Çalışma zamanında objenin gerçek tipi baz alınır -
 * POLİMORFİZM)
 * 
 * ==========================================
 */
